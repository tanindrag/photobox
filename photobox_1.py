...
class PhotoboxApp(QMainWindow):
    ...
    def start_photo_session(self):
        ...
        self.timer.start(5000)  # Capture a photo every 5 seconds
    ...
    def update_feed(self):
        ...
        if self.countdown > 0 and self.photo_index < 6:
            self.countdown -= 1
        elif self.photo_index < 6:
            self.countdown = 5  # Reset for the next photo
    ...
    def send_to_printer(self):
        """Send the captured photos to the default printer."""
        ...
        hdc.StartDoc("Photobox Print")
        hdc.StartPage()

        # Print photos (simulate by printing the file names)
        for photo in self.photos_taken:
            hdc.TextOut(100, 100, f"Photo: {photo}")
